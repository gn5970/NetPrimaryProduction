...
for j in range(37*135):
    ...
    # remove NaNs for checking
    valid = np.isfinite(series)

    if valid.sum() < 3:
        persistence[j, :] = np.nan
        continue

    series_valid = series[valid]

    # skip constant series
    if np.nanstd(series_valid) == 0:
        persistence[j, :] = 0
        continue
    acorr = sm.tsa.acf(series_valid, nlags=len(lags) - 1, fft=False)
    persistence[j, :] = np.nan_to_num(acorr, nan=0.0)

#persistence = persistence.reshape(nlat, nlon, nlags)
...
for i in range(1,20,6):
    ...
    plt.savefig("Figure6_a_d_g_persistence_NPP_"+str(i)+".png")
...
for k in range(1):
  ...
  for i in range(1, max_lead_time + 1,6):  # for lag = 1, 7
      ...
      plt.savefig("Figure6_b_e_h_linear_regression_"+str(i)+"_"+str(B[k])+".png")
  ...
  for i in range(1, max_lead_time + 1,6):  # for lag = 1, 7
      ...
      cb.ax.yaxis.set_major_formatter(mticker.FormatStrFormatter('%.1f'))
      plt.savefig("Supplemental_linear_regression_rmse_"+str(i)+"_"+str(B[k])+".png")
...
for k in range(1):
  ...
  for i in range(1,20,6):  # for lag = 1, 7
      ...
      plt.savefig("Supplemental_kernel_regression_rmse_"+str(i)+"_"+str(B[k])+".png")  
